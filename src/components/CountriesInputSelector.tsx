...
export const CountriesRegionSelector = ({filterByRegion, regions}:CountriesRegionSelectorProps) => {
  ...
  return (
            <div className="max-w-[480px] flex flex-col gap-2">
                <button className="px-12 flex items-center justify-between rounded-xl shadow-md min-h-24 bg-white dark:bg-darkBlueDark cursor-pointer font-medium"
                ...
                <div ref={menuRef} className={`hidden flex-col bg-white dark:bg-darkBlueDark shadow-md rounded-xl px-6 py-8`}>
                    {
                    regions.map((region)=>(
                        <a key={region} href='#' 
